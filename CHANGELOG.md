...
- Bumped iOS to `1.2.10 (22)` and Mac to `1.2.12 (30)` for the saved Remote Access Mac UX update and automatic Mac-side HelloX Admin reports when an outdated or malformed iOS client attempts to connect.
- Rebuilt and resubmitted iOS build `22` with Xcode `26.5` / iOS SDK `26.5` to replace rejected build `21`.
...
- Improved remote-control UX: native iOS remote screens show a clear "Connecting..." overlay while waiting for video, compact iPhone toolbar controls now meet 44pt touch targets, Touch/Trackpad mode changes give haptic feedback, and the Mac menu shows clearer Accessibility/copy/connection-state feedback.
- Reduced first-screen noise in AirDesk iOS by replacing the prominent Mac download card with a compact first-time setup hint that can be hidden manually and disappears automatically after a successful Local or Remote AirDesk connection.

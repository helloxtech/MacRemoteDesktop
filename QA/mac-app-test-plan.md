...
| CF-09 | Remote Access client version gate | Connect to the tunnel through an iOS client older than 1.2.5 | Mac rejects the tunnel connection, asks the user to update the iOS app, and sends one throttled HelloX Admin compatibility report with recent Mac-side diagnostics |

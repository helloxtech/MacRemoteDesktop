...
public extension VNCConnection {
	...
	func updateFramebufferRequestRegion(x: UInt16, y: UInt16, width: UInt16, height: UInt16) {
		guard width > 0, height > 0 else { return }

		state.framebufferRequestRegion = VNCRegion(
			x: x,
			y: y,
			width: width,
			height: height
		...
		clientToServerMessageQueue.enqueue(request)
	}

	func clearFramebufferRequestRegion() {
		state.framebufferRequestRegion = nil
		state.incrementalUpdatesEnabled = false
		state.areContinuousUpdatesEnabled = false

...
- Before any App Store, TestFlight, Mac direct-download, or HelloX website release, read `RELEASE_TODO.md`. Treat every unchecked item as release-blocking unless the user explicitly removes or defers it. Complete the item, verify it, then remove it from `RELEASE_TODO.md`; keep the file even when no items remain.

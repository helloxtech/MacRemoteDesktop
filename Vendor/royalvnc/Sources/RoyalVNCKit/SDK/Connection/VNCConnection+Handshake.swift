...
private extension VNCConnection {
	...
	func decideSecurityType(supportedTypes: VNCProtocol.SecurityTypes) async throws {
		...
		let preferredOrder: [VNCProtocol.SecurityType]
		switch settings.authenticationPreference {
			case .preferVNCPassword:
				preferredOrder = [.none, .vnc, .diffieHellman, .ultraVNCMSLogonII, .tight]
			case .preferUsernamePassword, .automatic:
				preferredOrder = [.none, .diffieHellman, .ultraVNCMSLogonII, .vnc, .tight]
		}

		chosenSecurityType = preferredOrder.first(where: { supportedSecurityTypes.contains($0) }) ?? .invalid
